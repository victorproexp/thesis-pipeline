...
# TF-IDF APPROACH with Minimal Stopwords
# TF-IDF naturally downweights common words via statistical weighting.
# Add only company/product names to prevent brand noise from obscuring topics.

minimal_stopwords = [
    # Company names & product brands
...
# ==========================================
# CUSTOM TOKENIZER (simplified for TF-IDF)
# ==========================================
# Note: TfidfVectorizer handles stopword filtering internally,
# so tokenizer just does basic cleaning
...
if __name__ == "__main__":
    ...
    # Combine NLTK stopwords with minimal company/product stopwords
    nltk_stopwords = set(stopwords.words('english'))
    combined_stopwords = list(nltk_stopwords.union(set(minimal_stopwords)))

    # TfidfVectorizer: TF-IDF weighting automatically handles term frequency weighting
    # Common words across corpus are naturally downweighted, rare discriminative words boosted
    vectorizer_model = TfidfVectorizer(
        tokenizer=custom_tokenizer,
        ngram_range=(1, 2),
        min_df=2,             # Word must appear in ≥2 documents
        max_df=0.75,          # Word can appear in ≤75% of documents (more aggressive with TF-IDF)
        norm='l2',            # L2 normalization (default, good for cosine similarity)
        sublinear_tf=True,    # Sublinear TF scaling (dampens term frequency influence)
        stop_words=combined_stopwords  # NLTK + minimal company/product names
    )
